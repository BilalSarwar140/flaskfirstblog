...
app = Flask(__name__)
app.config['SERVER_NAME'] = 'localhost:1000'
...
def dashboard():
    ...
    if request.method=='POST':
        ...
        if (username == params['admin_username'] and userpass == params['admin_password']):
            ...
            posts = Posts.query.all()
            return render_template('dashboard.html', params=params, posts= posts)
    return render_template('zlog.html', params=params)
...
def contact():
    if request.method == 'POST':
        ...
        db.session.commit()
        mail.send_message("Blog from " + name, sender=email, recipients = [params["gmail_username"]], body=message+"\n" + "Phone no: "+phone)
